...
trueFalse.controller("trueFalseCtrl", function($scope, $ionicPopup,  multiplyFunctions){
   ...
   //Funtion to call for trying again in setTimeOut
    ...
    function truefalseFunction(){
        ...
        tfData.currentIndex = tfData.oldArr.length; 
        tfData.randomIndex = 0;
        tfData.temporaryValue = 0;

        //Randomize the array
        while(0 !== tfData.currentIndex){
            tfData.randomIndex = Math.floor(Math.random() * tfData.currentIndex);
            tfData.currentIndex -= 1;
            tfData.temporaryValue = tfData.oldArr[tfData.currentIndex];
            tfData.oldArr[tfData.currentIndex] =  tfData.oldArr[tfData.randomIndex];
            tfData.oldArr[tfData.randomIndex] = tfData.temporaryValue ;
        }
